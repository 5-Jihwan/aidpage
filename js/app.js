...
import { t, getLang, setLang, applyStatic } from './i18n.js?v=20260906a';
import { initGrid, hasGrid, meta as gridMeta, cells as gridCells, available as gridAttrs, show as showGrid, hide as hideGrid, fmt as gridFmt, setExtrude as setGridExtrude, ATTRS as GRID_ATTRS } from './grid.js?v=20260906a';
...
async function openSimulator() {
  ...
  try {
    if (!_simMod) {
      _simMod = await import('./sim.js?v=20260906a');
      _simMod.initSim({
...
function initPanel() {
  ...
  ps.addEventListener('touchmove', e => {
    ...
    if (!sheetDrag && pullAtTop && dy > 12) { sheetDrag = true; moved = true; h0 = p.getBoundingClientRect().height; y0 = lastY = e.touches[0].clientY; p.style.transition = 'none'; }
...
function initPWA() {
  if ('serviceWorker' in navigator) navigator.serviceWorker.register('sw.js?v=20260906a').catch(() => {});
...
let _regionMod = null;
const regionMod = () => _regionMod || (_regionMod = import('./region.js?v=20260906a'));
