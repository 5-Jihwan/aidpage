...
import { t, getLang } from './i18n.js?v=20260902f';
...
function compute(payload) {
  return new Promise((resolve, reject) => {
    try {
      if (!worker) worker = new Worker('js/sim_worker.js?v=20260902f');
...
function resultHTML() {
  ...
  const gain = t('sim.r.gain', { min: Math.max(0, R.safe.walk_min - R.short.walk_min), a: R.short.sum.flood + R.short.sum.lslide, b: R.safe.sum.flood + R.safe.sum.lslide });
